...
export default async function LeaguePage({ params }: { params: Promise<{ code: string }> }) {
  ...
  for (const match of docs) { const item = grouped.get(match.matchday) ?? { number: match.matchday, count: 0, active: false, completed: true, live: false }; item.count++; item.completed = item.completed && match.status === "FINISHED"; item.live = item.live || match.status === "LIVE"; item.active = item.active || item.live || (match.status === "SCHEDULED" && new Date(match.kickoffAt).getTime() > Date.now()); grouped.set(match.matchday, item); }
