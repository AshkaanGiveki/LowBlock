...
export default async function RoundPage({ params }: { params: Promise<{ code: string; matchday: string }> }) {
  ...
  const canonical = await getCanonicalLeaderboard(db, { leagueCode: code, seasonStartYear: year, matchday: day }, 100);
  const ids = canonical.map((row) => row.userId);
  const predictions = await db.collection<any>("predictions").find({ userId: { $in: ids }, matchId: { $in: matches.map((match) => match.providerMatchId) } }).toArray();
  const scores = await db.collection<any>("predictionScores").find({ userId: { $in: ids }, matchId: { $in: matches.map((match) => match.providerMatchId) } }).toArray();
  const scoreMap = new Map(scores.map((score) => [`${score.userId}:${score.matchId}`, score]));
  const users = new Map(canonical.map((row) => [row.userId, row]));
  const grouped = new Map<string, any>();
  for (const prediction of predictions) {
    const match = matches.find((item) => item.providerMatchId === prediction.matchId);
    if (!match) continue;
    const locked = isPredictionLocked(match);
    const score = scoreMap.get(`${prediction.userId}:${prediction.matchId}`);
    const row = grouped.get(prediction.userId) ?? { userId: prediction.userId, username: users.get(prediction.userId)?.username ?? "LowBlock Player", avatarUrl: users.get(prediction.userId)?.avatarUrl ?? null, points: 0, predictions: [] };
    row.points += locked ? Number(score?.points ?? 0) : 0;
    row.predictions.push({ matchId: prediction.matchId, kickoffAt: new Date(match.kickoffAt).toISOString(), homeTeam: match.homeTeam, awayTeam: match.awayTeam, predictedHome: locked ? prediction.homeGoals : null, predictedAway: locked ? prediction.awayGoals : null, actualHome: locked ? match.homeGoals : null, actualAway: locked ? match.awayGoals : null, points: locked ? Number(score?.points ?? 0) : null, revealed: locked });
