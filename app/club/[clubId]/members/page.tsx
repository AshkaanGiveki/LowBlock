...
export default function ClubMembers({ params }: { params: Promise<{ clubId: string }> }) {
  ...
  return <main className="min-h-screen px-5 pb-28 pt-24 md:px-8 md:pt-32"><div className="mx-auto max-w-2xl">
    <ConfirmModal open={Boolean(removeTarget)} title={t("حذف این عضو؟", "Remove this member?")} description={t("این عضو از باشگاه شما خارج می‌شود، اما سابقه و امتیازهای جهانی او حفظ می‌شود.", "This member will leave your Club, while their global history and points remain safe.")} confirmLabel={t("حذف عضو", "Remove member")} cancelLabel={t("انصراف", "Cancel")} onConfirm={() => { const target = removeTarget; setRemoveTarget(null); if (target) void performRemove(target); }} onCancel={() => setRemoveTarget(null)} danger />
