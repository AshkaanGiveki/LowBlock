...
async function leaderboard(): Promise<{ rows: Row[]; usersCount: number; picksCount: number; roundsCount: number }> {
  ...
  const [rows, usersCount, picksCount, roundsCount] = await Promise.all([getCanonicalLeaderboard(db, { seasonStartYear: year }, 20), db.collection("users").countDocuments(), db.collection("predictions").countDocuments({ userId: { $ne: "guest" } }), db.collection("rounds").countDocuments({ status: "FINAL" })]);
  return { rows, usersCount, picksCount, roundsCount };
}

export default async function LeaderboardPage() {
  const [data, me] = await Promise.all([leaderboard(), currentUserId()]);
  return <Suspense fallback={<div className="min-h-screen animate-pulse p-8"><div className="mx-auto h-96 max-w-6xl rounded-3xl bg-white/[.06]" /></div>}><LeaderboardView {...data} me={me} /></Suspense>;
}
