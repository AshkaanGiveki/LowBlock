...
export async function GET(req: Request) {
  ...
  let cursor: { points: number; exact: number; correctOutcome?: number; globalPoints?: number; earliestPredictionAt?: number; predictions: number; userId: string } | undefined;
  if (encoded) { try { cursor = JSON.parse(Buffer.from(encoded, "base64url").toString("utf8")); } catch { return NextResponse.json({ error: "Invalid cursor" }, { status: 400 }); } }
