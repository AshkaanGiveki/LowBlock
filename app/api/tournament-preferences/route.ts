...
import {
  ...
  setShowAllMatches,
  updateTournamentPreference,
} from "@/lib/football/preferences";
...
const body = z
  ...
  .refine(
    (value) =>
      (value.leagueCode !== undefined && value.favorite !== undefined) ||
      value.showAllMatches !== undefined,
...
export async function GET() {
  ...
  return NextResponse.json({
    ...(await getTournamentPreferences(userId)),
    authenticated: true,
  });
}
...
export async function PUT(request: Request) {
  ...
  try {
    const preferences =
      parsed.data.showAllMatches !== undefined
        ? await setShowAllMatches(userId, parsed.data.showAllMatches)
        : await updateTournamentPreference(
            userId,
            parsed.data.leagueCode!,
