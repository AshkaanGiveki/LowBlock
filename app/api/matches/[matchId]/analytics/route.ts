...
export async function GET(req: Request, { params }: { params: Promise<{ matchId: string }> }) {
  ...
  const distribution = new Map<string, number>(); for (const prediction of visiblePredictions) { const key = `${prediction.homeGoals}-${prediction.awayGoals}`; distribution.set(key, (distribution.get(key) ?? 0) + 1); }
  const championId = await getDefendingChampionUserId(db);
  const userRows = visiblePredictions.map(prediction => { const score = scoreByUser.get(prediction.userId); return { userId: prediction.userId, username: names.get(prediction.userId)?.username ?? "LowBlock Player", avatarUrl: names.get(prediction.userId)?.avatarUrl ?? null, isDefendingChampion: prediction.userId === championId, submitted: true, homeGoals: prediction.homeGoals, awayGoals: prediction.awayGoals, points: locked ? Number(score?.points ?? 0) : null }; });
  return NextResponse.json({ match: { homeTeam: match.homeTeam, awayTeam: match.awayTeam, homeGoals: match.homeGoals, awayGoals: match.awayGoals, leagueCode: match.leagueCode, matchday: match.matchday, status: match.status, elapsed: match.elapsed ?? null, kickoffAt: new Date(match.kickoffAt).toISOString() }, locked, total: visiblePredictions.length, averagePoints: userRows.length ? userRows.reduce((sum, row) => sum + Number(row.points ?? 0), 0) / userRows.length : 0, distribution: [...distribution.entries()].map(([score, count]) => ({ score, count })).sort((a, b) => b.count - a.count), users: userRows });
}
