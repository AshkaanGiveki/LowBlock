...
export async function POST(req: Request) {
  ...
  if (
    ...
    env.TELEGRAM_ADMIN_CHAT_ID === String(chatId)
  ) {
    if (
  ...
  if (text.startsWith("/start ")) {
    ...
    try {
      ...
      await consumeLinkToken(token, "TELEGRAM", userId, {
      ...
      if (
        ...
        error.message === "PLATFORM_IDENTITY_CONFLICT"
      )
        await send(
          String(chatId),
          language === "fa"
            ? "این حساب تلگرام قبلاً به یک حساب LowBlock دیگر متصل شده است."
            : "This Telegram account is already connected to another LowBlock account.",
        );
      else
        await send(
          String(chatId),
          language === "fa"
            ? "لینک اتصال نامعتبر یا منقضی شده است. از تنظیمات LowBlock دوباره شروع کنید."
            : "This connection link is invalid or expired. Start again from LowBlock settings.",
        );
    }
    return NextResponse.json({ ok: true });
  }
...
async function handleCommand(
  ...
  if (!identity?.userId) {
    await send(
      chatId,
      "برای استفاده از ربات، ابتدا حساب تلگرام خود را به LowBlock متصل کنید.\n\nConnect your Telegram account to LowBlock first to use the bot.",
      connectKeyboard(),
    );
    return;
  }
  ...
  if (command === "language") {
    ...
    await (
      ...
      .updateOne(
        ...
        { $set: { language: nextLanguage, updatedAt: new Date() } },
        { upsert: true },
      );
    await send(
      chatId,
  ...
  if (command === "weekly") {
    ...
    await send(
      chatId,
      formatLeaderboard(rows, language, "weekly"),
  ...
  if (command === "group") {
    ...
    const membership = await db
      ...
      .findOne(
        { userId: identity.userId, leftAt: null },
        { projection: { clubId: 1 } },
      );
          ...
          : "You are not a member of a group yet.",
        {
          inline_keyboard: [
            [
              greenButton({
                text:
                  language === "fa" ? "👥 کشف گروه‌ها" : "👥 Discover groups",
                web_app: { url: `${env.NEXT_PUBLIC_APP_URL}/club/discover` },
              }),
            ],
          ],
        },
    ...
    const club = ObjectId.isValid(membership.clubId)
      ? await db
    ...
    const rows = await getCanonicalLeaderboard(
      db,
      {
        ...
        seasonStartYear: Number(
          latest?.seasonStartYear ?? new Date().getUTCFullYear(),
        ),
      },
      10,
    );
    await send(
      chatId,
      formatLeaderboard(rows, language, "group", club?.name),
      mainKeyboard(language),
    );
    return;
  }
  ...
  const db = await getDb();
  const user = ObjectId.isValid(identity.userId)
    ? await db
        .collection<any>("users")
        .findOne(
          { _id: new ObjectId(identity.userId) },
  ...
  const buttons: Record<string, InlineKeyboard> = {
    matches: {
      inline_keyboard: [
        [
          greenButton({
            text:
              language === "fa" ? "⚽ مسابقه‌های امروز" : "⚽ Today’s matches",
            web_app: { url: `${url}/matches` },
          }),
        ],
      ],
    },
    my_predictions: {
      inline_keyboard: [
        [
          greenButton({
            text:
              language === "fa" ? "🎯 پیش‌بینی‌های من" : "🎯 My predictions",
    ...
    results: {
      inline_keyboard: [
        [
          greenButton({
            text: language === "fa" ? "📊 مشاهده نتایج" : "📊 View results",
            web_app: { url: `${url}/matches?tab=results` },
          }),
        ],
      ],
    },
    ...
    settings: {
      inline_keyboard: [
        [
          greenButton({
            text:
              language === "fa" ? "⚙️ باز کردن تنظیمات" : "⚙️ Open settings",
            web_app: { url: profileUrl },
          }),
        ],
      ],
    ...
    language: {
      inline_keyboard: [
        [
          greenButton({
            text:
              language === "fa" ? "🌐 Language settings" : "🌐 تنظیمات زبان",
            web_app: { url: profileUrl },
          }),
        ],
      ],
  ...
  const messages: Record<string, [string, string]> = {
    matches: [
      "⚽ مسابقه‌های امروز\n\nبرای دیدن مسابقه‌ها و ثبت پیش‌بینی، دکمه زیر را انتخاب کنید.",
      "⚽ Today’s matches\n\nOpen today’s fixtures and submit your predictions below.",
    ],
    my_predictions: [
      "🎯 پیش‌بینی‌های شما در پروفایل LowBlock قابل مشاهده است.",
      "🎯 Your predictions are available in your LowBlock profile.",
    ],
    results: [
      "📊 نتایج و امتیازهای مسابقه‌ها را در LowBlock ببینید.",
      "📊 View match results and your scored predictions in LowBlock.",
    ],
    profile: [
      "👤 پروفایل، رتبه، امتیازها و افتخارات شما در LowBlock.",
      "👤 Your profile, rank, points, and awards on LowBlock.",
    ],
    reminders: [
      "🔔 وضعیت یادآوری‌ها را از پروفایل LowBlock مدیریت کنید.",
      "🔔 Manage your prediction reminders from your LowBlock profile.",
    ],
    settings: [
      "⚙️ تنظیمات حساب و اتصال‌های شما در LowBlock.",
      "⚙️ Manage your account settings and connected platforms in LowBlock.",
    ],
    language: [
  ...
  await send(
    chatId,
    language === "fa" ? messages[command][0] : messages[command][1],
    buttons[command],
  );
}
...
function mainKeyboard(language: "fa" | "en"): InlineKeyboard {
  return {
    inline_keyboard: [
      [
        greenButton({
          text: language === "fa" ? "⚽ مسابقه‌ها" : "⚽ Matches",
          web_app: { url: `${env.NEXT_PUBLIC_APP_URL}/matches` },
        }),
        greenButton({
          text: language === "fa" ? "🏆 جدول" : "🏆 Leaderboard",
          web_app: { url: `${env.NEXT_PUBLIC_APP_URL}/leaderboard` },
        }),
      ],
      [
        greenButton({
...
async function getTelegramIdentity(providerUserId: string) {
  return (await getDb())
    ...
    .findOne(
      { provider: "TELEGRAM", providerUserId },
      { projection: { userId: 1 } },
    );
}
function formatLeaderboard(
  ...
  const title =
    kind === "weekly"
      ? language === "fa"
        ? "🏆 جدول هفتگی جهانی LowBlock"
        : "🏆 LowBlock Global Weekly Leaderboard"
      : language === "fa"
        ? `👥 جدول گروه ${escapeHtml(groupName || "شما")}`
        : `👥 ${escapeHtml(groupName || "Your group")} Leaderboard`;
  const lines = rows.length
    ? rows
        .map(
          (row, index) =>
            `${["🥇", "🥈", "🥉"][index] ?? `${index + 1}.`} <b>${escapeHtml(row.username)}</b>  <code>${row.points} ${language === "fa" ? "امتیاز" : "pts"}</code>`,
        )
        .join("\n")
    : language === "fa"
...
async function sendLeaderboard(chatId: string, providerUserId: string) {
  ...
  if (!identity?.userId) {
    await send(
      chatId,
      "برای مشاهده جدول جهانی، ابتدا حساب تلگرام خود را به LowBlock متصل کنید.\n\nConnect your Telegram account to LowBlock first to view the global leaderboard.",
      true,
    );
    return;
  }
  ...
  const lines = rows.length
    ? rows
        .map(
          (row, index) =>
            `${medals[index] ?? `${index + 1}.`} <b>${escapeHtml(row.username)}</b>  <code>${row.points} ${language === "fa" ? "امتیاز" : "pts"}</code>`,
        )
        .join("\n")
    : "—";
  const header =
    language === "fa"
      ? `🏆 <b>جدول جهانی LowBlock</b>\n<i>فصل ${year}/${String(year + 1).slice(-2)} · ۱۰ نفر برتر</i>\n\n`
...
async function userForToken(token: string) {
  ...
  const row = await getDb().then((db) =>
    db
      .collection<{ userId: string }>("accountLinkTokens")
      .findOne(
        {
  ...
  if (!row) throw Error("LINK_TOKEN_INVALID");
  return row.userId;
}
async function userLanguage(userId: string) {
  const row = await getDb().then((db) =>
    db
      .collection<{ language?: string }>("notificationPreferences")
      .findOne({ userId }, { projection: { language: 1 } }),
  );
  return row?.language === "en" ? "en" : ("fa" as "fa" | "en");
}
...
async function send(
  chatId: string,
  text: string,
  keyboard: boolean | InlineKeyboard = false,
) {
            ...
            ? {
                inline_keyboard: [
                  [
                    greenButton({
                      text: "🏆 Leaderboard",
                      web_app: { url: `${env.NEXT_PUBLIC_APP_URL}/lowblock` },
                    }),
                    greenButton({
                      text: "⚽ Predict",
                      web_app: { url: `${env.NEXT_PUBLIC_APP_URL}/matches` },
