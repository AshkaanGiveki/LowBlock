...
export async function POST(req: Request) {
  try {
    ...
    const userId=result.insertedId.toString(); const pending=readPending((await cookies()).get("lowblock_platform_pending")?.value); if(pending) await attachIdentity(userId,pending.provider,pending.user); const session = await createSession(userId); const response = NextResponse.json({ ok: true }, { status: 201 });
