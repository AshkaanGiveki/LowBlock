...
export async function POST(req: Request) {
  try {
    ...
    const pending=readPending((await cookies()).get("lowblock_platform_pending")?.value); const result = await db.collection("users").insertOne({ username, normalizedUsername: normalizeUsername(username), passwordHash: await hashPassword(parsed.data.password), createdAt: now, updatedAt: now });
