...
export async function POST(req: Request) {
  ...
  const userId=String(user._id); const pending=readPending((await cookies()).get("lowblock_platform_pending")?.value); try { if(pending) await attachIdentity(userId,pending.provider,pending.user); } catch(error) { if(error instanceof Error&&error.message==="PLATFORM_IDENTITY_CONFLICT") return NextResponse.json({error:"This platform account is already linked to another LowBlock account."},{status:409}); throw error; } const session = await createSession(userId); const response = NextResponse.json({ ok: true });
