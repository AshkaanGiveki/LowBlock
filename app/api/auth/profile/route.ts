...
export async function PUT(request: Request) {
  ...
  const input = await request.json().catch(() => null) as { username?: unknown; password?: unknown; avatarUrl?: unknown } | null;
  ...
  if (avatarUrl && !/^https?:\/\//i.test(avatarUrl)) return NextResponse.json({ error: "Avatar must be a valid image URL" }, { status: 400 });
