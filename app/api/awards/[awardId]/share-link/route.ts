...
import {
  createAwardShareToken,
  hashAwardShareToken,
} from "@/lib/awards/shareToken";
...
export async function POST(
  ...
  const award = await db
    ...
    .findOne(filter, {
      projection: { shareCopyIndex: 1, shareToken: 1, shareTokenHash: 1 },
    });
  if (!award)
    return NextResponse.json({ error: "Award not found" }, { status: 404 });
  const locale =
    new URL(request.url).searchParams.get("locale") === "en" ? "en" : "fa";
  if (
    typeof award.shareToken === "string" &&
    /^[A-Za-z0-9_-]{6}$/.test(award.shareToken)
  )
    return NextResponse.json({
      url: `https://lowblock.ir/s/${award.shareToken}`,
      message: getAwardShareMessage(locale, award.shareCopyIndex),
    });
  for (let attempt = 0; attempt < 8; attempt++) {
    const token = createAwardShareToken();
    const tokenTaken = await db
      .collection("awards")
      .findOne(
        { shareTokenHash: hashAwardShareToken(token) },
        { projection: { _id: 1 } },
      );
    if (tokenTaken) continue;
