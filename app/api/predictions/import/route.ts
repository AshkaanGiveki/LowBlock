...
export async function POST(req: Request) {
  ...
  if (!parsed.success)
    return NextResponse.json({ error: "INVALID_RECEIPTS" }, { status: 400 });
  const db = await getDb();
  const imported: string[] = [];
  const handled: string[] = [];
  const discarded: string[] = [];
  for (const receipt of parsed.data.receipts) {
    const payload = verifyGuestReceipt(receipt);
    if (!payload) {
    ...
    if (
      !match ||
      payload.issuedAt > new Date(match.kickoffAt).getTime() + 60_000
    )
      continue;
    const existing = await db
      .collection("predictions")
      .findOne({ userId, matchId: payload.matchId });
    if (!existing) {
      await upsertPrediction(db, userId, payload);
      imported.push(payload.matchId);
    }
    handled.push(payload.matchId);
  }
