...
export async function POST(req: Request) {
  ...
  if (isPredictionLocked(match))
    return NextResponse.json({ error: "PREDICTION_LOCKED" }, { status: 409 });
  const now = new Date();
  await db
    .collection("predictions")
    .updateOne(
      { userId, matchId: parsed.data.matchId },
      {
        $set: { ...parsed.data, userId, updatedAt: now },
        $setOnInsert: { createdAt: now },
      },
      { upsert: true },
    );
  if (typeof match.leagueCode === "string" && ObjectId.isValid(userId))
    await db
      .collection("users")
      .updateOne(
        { _id: new ObjectId(userId) },
        {
          $addToSet: { favoriteTournamentIds: match.leagueCode },
          $pull: { favoriteTournamentExclusions: match.leagueCode },
          $set: { updatedAt: now },
        },
      );
