...
export async function POST(req: Request) {
  ...
  const invite = await db
    ...
    .findOne({ tokenHash: hash(token), kind: "CLUB_LINK", status: "ACTIVE" });
  if (!invite)
    return NextResponse.json(
      { error: "INVITATION_REVOKED_OR_INVALID" },
      { status: 404 },
    );
  const existing = await currentMembership(db, userId);
  if (existing?.clubId === invite.clubId)
    return NextResponse.json({ error: "ALREADY_MEMBER" }, { status: 409 });
  if (existing && !confirmSwitch)
    return NextResponse.json(
      { requiresSwitchConfirmation: true, currentClubId: existing.clubId },
      { status: 409 },
    );
  if (existing) {
    const currentClub = await db
      .collection<any>("clubs")
      .findOne({ _id: new ObjectId(existing.clubId) });
    if (currentClub?.ownerId === userId)
      return NextResponse.json(
