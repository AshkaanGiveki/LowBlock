  ...
  try {
    await withMongoTransaction(async (transactionDb, session) => {
        ...
        .findOne(
          {
            ...
            discoveryMode: "RECRUITING",
            visibility: "PUBLIC",
          },
          { session },
        );
      if (!club) throw new Error("CLUB_NOT_RECRUITING");
      const membership = await transactionDb
      ...
      await transactionDb
        ...
        .insertOne(
          { clubId, userId, status: "PENDING", createdAt: now, updatedAt: now },
          { session },
        );
    });
    return NextResponse.json(
      { ok: true, status: "PENDING", clubId },
      { status: 201 },
    );
  } catch (error) {
    const code = error instanceof Error ? error.message : "JOIN_REQUEST_FAILED";
    const status = ["ALREADY_MEMBER", "REQUEST_ALREADY_PENDING"].includes(code)
      ? 409
      : code === "CLUB_NOT_RECRUITING"
        ? 404
        : 400;
