...
export async function GET(
  ...
  return NextResponse.json({
    ...
    members: members.map((member) => ({
      ...member,
      isDefendingChampion: String(member.userId) === championId,
    })),
    isOwner: club.ownerId === viewer,
  });
}
...
export async function DELETE(
  ...
  const result = await db
    .collection("clubMemberships")
    .updateOne(
      { clubId, userId: targetUserId, leftAt: null },
      { $set: { leftAt: new Date() } },
    );
  if (!result.matchedCount)
    return NextResponse.json({ error: "ALREADY_MEMBER" }, { status: 404 });
...
export async function PATCH(
  ...
  if (!actor || !ObjectId.isValid(clubId))
    return NextResponse.json({ error: "AUTH_REQUIRED" }, { status: 401 });
  const data = await req.json().catch(() => ({}));
  if (data.action !== "TRANSFER" || typeof data.userId !== "string")
    return NextResponse.json(
