...
import { BarChart3, Check, Crown, Minus, Plus, Radio, Sparkles, X } from "lucide-react";
...
export function PredictionCard({ match, initial, index = 0, onDrawerChange }: Props) {
  ...
  async function submit() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error ?? t("ذخیره نشد", "Prediction could not be saved"));
      setSaved(true); closeDrawer();
    } catch (reason) { setError(reason instanceof Error ? reason.message : t("خطایی رخ داد", "Something went wrong")); } finally { setBusy(false); }
  }
  const statusText = live ? "LIVE" : finished ? `FT${hasResult ? ` · ${formatNumber(match.homeGoals ?? 0, language)} - ${formatNumber(match.awayGoals ?? 0, language)}` : ""}` : "UPCOMING";
  return <>
...
function PredictionDrawer({ home, away, homeValue, awayValue, setHome, setAway, onClose, onSubmit, busy, saved, error, language }: { home: DrawerTeam; away: DrawerTeam; homeValue: string; awayValue: string; setHome: (value: string) => void; setAway: (value: string) => void; onClose: () => void; onSubmit: () => void; busy: boolean; saved: boolean; error: string; language: "fa" | "en" }) {
  const t = (fa: string, en: string) => language === "fa" ? fa : en;
  const display = (value: string) => value === "" ? "—" : formatNumber(Number(value), language);
  return <div className="fixed inset-0 z-[80] md:hidden"><button aria-label={t("بستن", "Close")} onClick={onClose} className="absolute inset-0 bg-black/75 backdrop-blur-md"/><motion.div initial={{ y: "100%", opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: "100%", opacity: 0 }} transition={{ type: "spring", stiffness: 300, damping: 30 }} className="absolute inset-x-0 bottom-0 overflow-hidden rounded-t-[2.25rem] border border-b-0 border-white/10 bg-[#0d1712] px-5 pb-8 pt-14 shadow-[0_-30px_100px_rgba(0,0,0,.72)]"><div className="absolute left-1/2 top-3 h-1.5 w-12 -translate-x-1/2 rounded-full bg-white/20"/><button onClick={onClose} aria-label={t("بستن", "Close")} className="absolute left-5 top-5 grid h-10 w-10 place-items-center rounded-2xl border border-white/10 bg-white/[.07] text-white/75"><X size={18}/></button><div className="mb-7 text-center"><div className="mx-auto mb-3 inline-flex items-center gap-2 rounded-full border border-brand/25 bg-brand/10 px-3 py-1.5 text-[10px] font-black tracking-[.18em] text-brand"><BarChart3 size={13}/>{t("پیش‌بینی مسابقه", "MATCH PREDICTION")}</div><h2 className="text-2xl font-black">{t("نتیجه‌ات را ثبت کن", "Lock in your score")}</h2><p className="mt-2 text-xs text-white/50">{t("امتیاز خود را با انتخاب گل‌های هر تیم بساز.", "Choose both teams’ goals before kick-off.")}</p></div><div className="relative grid grid-cols-2 gap-3"><DrawerTeamCard team={home} value={homeValue} setValue={setHome} language={language}/><DrawerTeamCard team={away} value={awayValue} setValue={setAway} language={language}/><div className="pointer-events-none absolute left-1/2 top-1/2 grid h-12 w-12 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-2xl border border-brand/30 bg-[#10231a] text-xs font-black text-brand shadow-xl">{display(homeValue)}<span className="mx-0.5 text-white/40">–</span>{display(awayValue)}</div></div>{error && <p className="mt-4 rounded-xl bg-red-950/30 p-3 text-center text-xs text-red-200">{error}</p>}<button disabled={busy || homeValue === "" || awayValue === ""} onClick={onSubmit} className="mt-6 flex w-full items-center justify-center gap-2 rounded-2xl bg-brand py-4 text-sm font-black text-white shadow-[0_12px_30px_rgba(32,184,121,.22)] disabled:opacity-45">{saved ? <><Check size={16}/>{t("ذخیره شد", "Saved")}</> : busy ? t("در حال ثبت…", "Saving…") : t("ثبت پیش‌بینی", "Save prediction")}</button></motion.div></div>;
}
function DrawerTeamCard({ team, value, setValue, language }: { team: DrawerTeam; value: string; setValue: (value: string) => void; language: "fa" | "en" }) { const n = (amount: number) => formatNumber(amount, language); return <div className="rounded-2xl border border-white/[.08] bg-gradient-to-b from-white/[.06] to-black/20 p-4 text-center"><div className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-white/[.05] p-2"><TeamCrest name={team.name} logo={team.logo}/></div><p className="mt-3 line-clamp-2 min-h-8 text-xs font-bold text-white/80">{team.name}</p><div className="mt-5 flex items-center justify-center gap-2"><button aria-label="Decrease score" disabled={Number(value || 0) <= 0} onClick={() => setValue(String(Math.max(0, Number(value || 0) - 1)))} className="grid h-9 w-9 place-items-center rounded-xl border border-white/10 bg-white/[.05] text-white/75 transition hover:border-brand hover:text-brand disabled:opacity-25"><Minus size={15}/></button><span className="grid h-12 w-12 place-items-center rounded-xl border border-brand/30 bg-brand/10 text-2xl font-black text-brand">{value === "" ? "—" : n(Number(value))}</span><button aria-label="Increase score" onClick={() => setValue(String(Math.min(20, Number(value || 0) + 1)))} className="grid h-9 w-9 place-items-center rounded-xl bg-brand text-white shadow-[0_6px_18px_rgba(32,184,121,.25)] transition hover:bg-brand-hover"><Plus size={15}/></button></div></div>; }
