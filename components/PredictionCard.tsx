  ...
  return <>
    <motion.article initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .4, delay: index * .05 }} whileHover={{ y: -4 }} onClick={preventCardOpen} className="glow-card relative mb-5 overflow-hidden rounded-2xl border border-brand/55 bg-transparent shadow-[0_18px_55px_rgba(0,0,0,.24)] transition-colors hover:border-brand">
      <div className="relative rounded-[15px] border border-brand/20 bg-transparent px-5 pb-14 pt-5 backdrop-blur-sm">
        ...
        {error && <p role="alert" className="mt-3 rounded-lg border border-red-400/30 bg-red-950/25 p-2 text-center text-[10px] text-red-100">{error}</p>}
      </div>
      <button type="button" disabled={busy || home === "" || away === "" || locked} onClick={submit} className="absolute bottom-2 left-1/2 hidden min-w-40 -translate-x-1/2 items-center justify-center gap-2 rounded-full border border-brand/60 bg-transparent px-5 py-2.5 text-xs font-black text-brand shadow-lg transition hover:-translate-y-0.5 hover:bg-brand/10 disabled:opacity-60 md:flex">{saved ? <><Check size={14}/>{t("ذخیره شد", "Saved")}</> : <><Sparkles size={14}/>{busy ? t("در حال ثبت…", "Saving…") : t("ثبت پیش‌بینی", "Save prediction")}</>}</button>
    </motion.article>
...
function PredictionDrawer({ home, away, homeValue, awayValue, setHome, setAway, onClose, onSubmit, matchId, insightsOpen, onToggleInsights, busy, saved, error, language }: { home: DrawerTeam; away: DrawerTeam; homeValue: string; awayValue: string; setHome: (value: string) => void; setAway: (value: string) => void; onClose: () => void; onSubmit: () => void; matchId: string; insightsOpen: boolean; onToggleInsights: () => void; busy: boolean; saved: boolean; error: string; language: "fa" | "en" }) {
  ...
  return <div className="fixed inset-0 z-[80] md:hidden"><button type="button" aria-label={t("بستن", "Close")} onClick={onClose} className="absolute inset-0 bg-black/75 backdrop-blur-md"/><motion.div initial={{ y: "100%", opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: "100%" }} transition={{ type: "spring", stiffness: 300, damping: 30 }} className="absolute inset-x-0 bottom-0 max-h-[94vh] overflow-y-auto rounded-t-[2.25rem] border border-b-0 border-brand/40 bg-[#0d1712]/95 px-5 pb-8 pt-14 shadow-[0_-30px_100px_rgba(0,0,0,.72)]"><div className="absolute left-1/2 top-3 h-1.5 w-12 -translate-x-1/2 rounded-full bg-brand/50"/><button type="button" onClick={onClose} aria-label={t("بستن", "Close")} className="absolute left-5 top-5 grid h-10 w-10 place-items-center rounded-2xl border border-brand/30 bg-transparent text-brand"><X size={18}/></button><div className="mb-7 text-center"><div className="mx-auto mb-3 inline-flex items-center gap-2 rounded-full border border-brand/35 bg-brand/10 px-3 py-1.5 text-[10px] font-black tracking-[.18em] text-brand"><BarChart3 size={13}/>{t("پیش‌بینی مسابقه", "MATCH PREDICTION")}</div><h2 className="text-2xl font-black text-white">{t("نتیجه‌ات را ثبت کن", "Lock in your score")}</h2><p className="mt-2 text-xs text-white/50">{t("گل‌های هر دو تیم را قبل از شروع بازی انتخاب کن.", "Choose both teams’ goals before kick-off.")}</p></div><div className="relative grid grid-cols-2 gap-3"><DrawerTeamCard team={home} value={homeValue} setValue={setHome} language={language}/><DrawerTeamCard team={away} value={awayValue} setValue={setAway} language={language}/><div className="pointer-events-none absolute left-1/2 top-1/2 grid h-12 w-12 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-2xl border border-brand/40 bg-[#10231a] text-xs font-black text-brand shadow-xl">{display(homeValue)}<span className="mx-0.5 text-white/40">–</span>{display(awayValue)}</div></div><div className="relative my-6"><div className="border-t border-dashed border-brand/35"/><button type="button" onClick={onToggleInsights} aria-expanded={insightsOpen} className="absolute left-1/2 top-1/2 inline-flex -translate-x-1/2 -translate-y-1/2 items-center gap-1.5 rounded-full border border-brand/40 bg-[#0d1712] px-3 py-1.5 text-[10px] font-black text-brand shadow-[0_0_0_5px_#0d1712] transition hover:bg-brand/10"><span>{t("فرم و جدول", "Form & table")}</span><ChevronDown size={14} className={insightsOpen ? "rotate-180 transition-transform" : "transition-transform"}/></button></div>{insightsOpen && <div className="mb-5"><MatchInsightsPanel matchId={matchId}/></div>}{error && <p className="mt-4 rounded-xl border border-red-400/30 bg-red-950/30 p-3 text-center text-xs text-red-200">{error}</p>}<button type="button" disabled={busy || homeValue === "" || awayValue === ""} onClick={onSubmit} className="mt-2 flex w-full items-center justify-center gap-2 rounded-2xl border border-brand/60 bg-brand py-4 text-sm font-black text-white shadow-[0_12px_30px_rgba(32,184,121,.22)] disabled:opacity-45">{saved ? <><Check size={16}/>{t("ذخیره شد", "Saved")}</> : busy ? t("در حال ثبت…", "Saving…") : t("ثبت پیش‌بینی", "Save prediction")}</button></motion.div></div>;
}

function DrawerTeamCard({ team, value, setValue, language }: { team: DrawerTeam; value: string; setValue: (value: string) => void; language: "fa" | "en" }) { const n = (amount: number) => formatNumber(amount, language); const isolate = (event: React.SyntheticEvent<HTMLButtonElement>) => event.stopPropagation(); return <div className="rounded-2xl border border-brand/25 bg-transparent p-4 text-center"><div className="mx-auto grid h-16 w-16 place-items-center rounded-full border border-brand/25 bg-transparent p-2"><TeamCrest name={team.name} logo={team.logo}/></div><p className="mt-3 line-clamp-2 min-h-8 text-xs font-bold text-white/80">{team.name}</p><div className="mt-5 flex items-center justify-center gap-2"><button type="button" aria-label="Decrease score" disabled={Number(value || 0) <= 0} onPointerDown={isolate} onClick={(event) => { isolate(event); setValue(String(Math.max(0, Number(value || 0) - 1))); }} className="grid h-9 w-9 place-items-center rounded-xl border border-brand/35 bg-transparent text-brand transition hover:bg-brand/10 disabled:opacity-25"><Minus size={15}/></button><span className="grid h-12 w-12 place-items-center rounded-xl border border-brand/40 bg-brand/10 text-2xl font-black text-brand">{value === "" ? "—" : n(Number(value))}</span><button type="button" aria-label="Increase score" onPointerDown={isolate} onClick={(event) => { isolate(event); setValue(String(Math.min(20, Number(value || 0) + 1))); }} className="grid h-9 w-9 place-items-center rounded-xl border border-brand/50 bg-transparent text-brand shadow-[0_6px_18px_rgba(32,184,121,.18)] transition hover:bg-brand/10"><Plus size={15}/></button></div></div>; }
