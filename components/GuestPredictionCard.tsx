...
import { PredictionDrawer } from "@/components/PredictionCard";
...
export function GuestPredictionCard({
  ...
  initial?: Prediction;
  onClose: () => void;
  onSaved: (localOnly: boolean, prediction: Prediction) => void;
}) {
  const { language, t } = useLanguage();
  const [home, setHome] = useState(String(initial?.homeGoals ?? 0));
  const [away, setAway] = useState(String(initial?.awayGoals ?? 0));
  ...
  async function submit() {
    ...
    try {
      ...
      const response = await fetch("/api/predictions/guest", {
        method: "POST",
      ...
      if (!response.ok)
        throw new Error(
          t("ثبت پیش‌بینی انجام نشد.", "Prediction could not be saved."),
        );
      const prediction = {
