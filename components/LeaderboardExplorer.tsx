...
export function LeaderboardExplorer({ initialRows, seasonStartYear, leagueCode, lifetime, weekly = false, me }: { initialRows: Row[]; seasonStartYear: number; leagueCode?: string; lifetime: boolean; weekly?: boolean; me: string | null }) {
  const [localPending, setLocalPending] = useState(false);
  useEffect(() => setLocalPending(false), [lifetime, weekly, leagueCode]);
  const loading = localPending;
  return <><LeaderboardFilters lifetime={lifetime} weekly={weekly} leagueCode={leagueCode} onNavigate={() => setLocalPending(true)} />{loading ? <LeaderboardSkeleton /> : <InfiniteLeaderboard key={`${weekly ? "weekly" : lifetime ? "lifetime" : "season"}:${leagueCode ?? "all"}`} initialRows={initialRows} seasonStartYear={seasonStartYear} leagueCode={leagueCode} lifetime={lifetime} weekly={weekly} me={me}/>}</>;
}
