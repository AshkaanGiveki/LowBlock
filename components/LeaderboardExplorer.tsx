...
export function LeaderboardExplorer({ initialRows, seasonStartYear, leagueCode, lifetime, weekly = false, weekOffset = 0, me }: { initialRows: Row[]; seasonStartYear: number; leagueCode?: string; lifetime: boolean; weekly?: boolean; weekOffset?: number; me: string | null }) {
  ...
  useEffect(() => setLocalPending(false), [lifetime, weekly, weekOffset, leagueCode]);
  const loading = localPending;
  return <><LeaderboardFilters lifetime={lifetime} weekly={weekly} weekOffset={weekOffset} leagueCode={leagueCode} onNavigate={() => setLocalPending(true)} />{loading ? <LeaderboardSkeleton /> : <InfiniteLeaderboard key={`${weekly ? `weekly:${weekOffset}` : lifetime ? "lifetime" : "season"}:${leagueCode ?? "all"}`} initialRows={initialRows} seasonStartYear={seasonStartYear} leagueCode={leagueCode} lifetime={lifetime} weekly={weekly} weekOffset={weekOffset} me={me}/>}</>;
}
