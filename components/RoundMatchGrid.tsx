  ...
  return <>
    <section className="mt-7">
      ...
      <div className="grid gap-3 sm:grid-cols-2">{fixtures.map((rawFixture) => { const fixture = LIVE_SCORE_UI_ENABLED ? rawFixture : { ...rawFixture, status: rawFixture.status === "LIVE" ? "" : rawFixture.status };
        ...
        return <button key={fixture.providerMatchId} data-match-id={fixture.providerMatchId} disabled={!started} onClick={() => { if (!started) return; openMatchRoute(fixture.providerMatchId, window.location.pathname); setSelected(fixture.providerMatchId); }} className={`group relative overflow-hidden rounded-[1.35rem] border border-white/[.08] bg-[linear-gradient(145deg,#14221a,#0d120f)] p-4 text-start transition ${started ? "hover:-translate-y-1 hover:border-brand/50 hover:shadow-[0_18px_45px_rgba(32,184,121,.14)]" : "cursor-default opacity-90"}`}>
