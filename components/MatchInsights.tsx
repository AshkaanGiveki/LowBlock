...
export type MatchInsightsData = { home: InsightTeam | null; away: InsightTeam | null; h2h: H2HMeeting[] };
...
export function MatchInsightsPanel({ matchId }: { matchId: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/matches/${encodeURIComponent(matchId)}/insights`)
      ...
      .then((value) => { if (mounted) setData({ home: value?.home ?? null, away: value?.away ?? null, h2h: Array.isArray(value?.h2h) ? value.h2h : [] }); })
  ...
  return <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="overflow-hidden"><H2HCard data={data.h2h} homeTeamId={data.home?.id ?? null} awayTeamId={data.away?.id ?? null} language={language} n={n} t={t} /></motion.div>;
}
...
function H2HCard({ data, homeTeamId, awayTeamId, language, n, t }: { data: H2HMeeting[]; homeTeamId: number | null; awayTeamId: number | null; language: "fa" | "en"; n: (value: number) => string; t: (fa: string, en: string) => string }) {
  const summary = data.reduce((acc, item) => { if (item.homeGoals === item.awayGoals) { acc.draw++; return acc; } const homeWon = homeTeamId === item.homeTeam.id ? item.homeGoals > item.awayGoals : homeTeamId === item.awayTeam.id ? item.awayGoals > item.homeGoals : false; const awayWon = awayTeamId === item.homeTeam.id ? item.homeGoals > item.awayGoals : awayTeamId === item.awayTeam.id ? item.awayGoals > item.homeGoals : false; if (homeWon) acc.home++; else if (awayWon) acc.away++; return acc; }, { home: 0, draw: 0, away: 0 });
  return <section className="overflow-hidden rounded-[1.35rem] border border-brand/20 bg-[radial-gradient(circle_at_90%_0%,rgba(32,184,121,.16),transparent_45%),rgba(0,0,0,.16)]"><div className="flex items-center gap-3 border-b border-white/[.07] p-3.5"><span className="grid h-10 w-10 place-items-center rounded-xl bg-brand/15 text-brand"><History size={19} /></span><div><h3 className="text-sm font-black">{t("پنج رویارویی آخر", "LAST FIVE MEETINGS")}</h3><p className="mt-1 text-[9px] text-[var(--muted)]">{t("سابقه مستقیم دو تیم", "Direct history between the teams")}</p></div><span className="ms-auto text-[10px] font-black text-[var(--muted)]">{n(data.length)} {t("بازی", "matches")}</span></div><div className="grid grid-cols-3 gap-2 p-3 text-center"><div className="rounded-xl bg-brand/10 p-2"><b className="block text-lg text-brand">{n(summary.home)}</b><small className="text-[9px] text-[var(--muted)]">{t("برد میزبان", "Home wins")}</small></div><div className="rounded-xl bg-white/[.04] p-2"><b className="block text-lg">{n(summary.draw)}</b><small className="text-[9px] text-[var(--muted)]">{t("مساوی", "Draws")}</small></div><div className="rounded-xl bg-red-400/10 p-2"><b className="block text-lg text-red-300">{n(summary.away)}</b><small className="text-[9px] text-[var(--muted)]">{t("برد مهمان", "Away wins")}</small></div></div><div className="px-4 pb-3">{data.length ? data.map((item) => <div key={item.fixtureId} className="flex items-center gap-2 border-t border-white/[.05] py-2.5">
    <TeamCrest name={item.homeTeam.name} logo={item.homeTeam.logoUrl} className="h-7 w-7 shrink-0" />
    <span className="min-w-0 flex-1 truncate text-[11px] font-bold">{teamName(language, item.homeTeam.id, item.homeTeam.name)}</span><b className="shrink-0 rounded-lg border border-white/10 bg-black/20 px-2.5 py-1.5 text-[11px]">{n(item.homeGoals)} - {n(item.awayGoals)}</b><span className="min-w-0 flex-1 truncate text-end text-[11px] font-bold">{teamName(language, item.awayTeam.id, item.awayTeam.name)}</span><span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-white/[.05] p-1"><TeamCrest name={item.awayTeam.name} logo={item.awayTeam.logoUrl} /></span><time className="hidden w-16 text-end text-[9px] text-[var(--muted)] sm:block">{new Date(item.date).toLocaleDateString(language === "fa" ? "fa-IR" : "en-GB", { month: "short", day: "numeric" })}</time></div>) : <p className="p-4 text-center text-xs text-[var(--muted)]">{t("سابقه رودررو موجود نیست.", "No head-to-head history available.")}</p>}</div></section>;
}
