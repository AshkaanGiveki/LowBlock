...
function PodiumCard({ row, rank, language, t }: { row: Row; rank: number; language: "fa" | "en"; t: (fa: string, en: string) => string }) { const place = formatNumber(rank, language); return <div className={`${rank === 1 ? "order-1 col-span-2 md:order-2 md:col-span-1 md:pb-8" : rank === 2 ? "order-2 md:order-1" : "order-3 md:order-3"}`}><div className={`rounded-2xl border p-5 text-center ${rank === 1 ? "border-[#e8c66a]/40 bg-[#e8c66a]/10" : "border-white/[.08] bg-[#111713]"}`}><div className="mb-3 flex items-center justify-center gap-2 text-xs font-black text-[var(--muted)]"><Medal size={15} className={rank === 1 ? "text-[#e8c66a]" : "text-brand"}/>{place}</div><Avatar row={row} large/><h2 className="mt-3 truncate font-black">{row.username}</h2><p className="mt-1 text-xs text-[var(--muted)]">{formatNumber(row.predictions, language)} {t("پیش‌بینی","picks")}</p><div className="mt-4 inline-flex items-center gap-2 rounded-lg bg-black/25 px-3 py-2 text-sm font-black text-brand"><Sparkles size={14}/>{formatNumber(row.points, language)}</div></div></div>; }
