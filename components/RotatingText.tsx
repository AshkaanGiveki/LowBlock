...
const RotatingText = forwardRef<RotatingTextHandle, RotatingTextProps>(
  function RotatingText(
    ...
    const next = useCallback(
      () =>
        setCurrent((value) => (value === safeTexts.length - 1 ? 0 : value + 1)),
      [safeTexts.length],
    );
    const previous = useCallback(
      () =>
        setCurrent((value) => (value === 0 ? safeTexts.length - 1 : value - 1)),
      [safeTexts.length],
    );
    const jumpTo = useCallback(
      (index: number) =>
        setCurrent(Math.max(0, Math.min(index, safeTexts.length - 1))),
      [safeTexts.length],
    );
    ...
    useLayoutEffect(() => {
      ...
      if (!measurement || !container) return;
      ...
      const boxExtras =
        styles.boxSizing === "border-box"
          ...
          : 0;
      setContentWidth(
        Math.ceil(measurement.getBoundingClientRect().width + boxExtras),
      );
    }, [current, safeTexts, splitBy]);
    const elements = useMemo(() => {
      const text = safeTexts[current] ?? "";
      if (splitBy === "lines") return text.split("\n").map((line) => [line]);
      if (splitBy === "words") return text.split(" ").map((word) => [word]);
    ...
    const delayFor = (index: number) =>
      staggerFrom === "last"
        ? (total - 1 - index) * staggerDuration
        : staggerFrom === "center"
          ? Math.abs(Math.floor(total / 2) - index) * staggerDuration
    ...
    return (
      <motion.span
        ...
        layout
        animate={{ width: contentWidth == null ? "auto" : contentWidth }}
        transition={{ type: "spring", damping: 30, stiffness: 400 }}
      >
        <span className="text-rotate-sr-only">{safeTexts[current]}</span>
        <span className="text-rotate-measure-layer" aria-hidden="true">
        ...
        <AnimatePresence mode="wait" initial={false}>
          <motion.span
            ...
            aria-hidden="true"
          >
            {elements.map((element, wordIndex) => (
              <span
                key={wordIndex}
                className={`text-rotate-word ${splitLevelClassName}`}
              >
                {element.map((part, partIndex) => (
                  <motion.span
                    key={partIndex}
                    initial={{ y: "100%" }}
                    animate={{ y: 0 }}
