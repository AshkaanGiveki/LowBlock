...
const RotatingText = forwardRef<RotatingTextHandle, RotatingTextProps>(function RotatingText({ texts, rotationInterval = 2200, staggerDuration = .025, staggerFrom = "last", splitBy = "characters", mainClassName = "", splitLevelClassName = "" }, ref) {
  ...
  useLayoutEffect(() => {
    ...
    const boxExtras = styles.boxSizing === "border-box"
      ? [styles.paddingLeft, styles.paddingRight, styles.borderLeftWidth, styles.borderRightWidth].reduce((total, value) => total + Number.parseFloat(value || "0"), 0)
      : 0;
    setContentWidth(Math.ceil(measurement.getBoundingClientRect().width + boxExtras));
