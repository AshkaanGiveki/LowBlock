...
export function Nav() {
  const pathname = usePathname(); const { language, setLanguage, t } = useLanguage(); const [user, setUser] = useState<User | null>(null);
  const authPage = pathname === "/login" || pathname === "/signup"; const appPage = pathname !== "/" && !authPage;
  ...
  return <><header className="fixed inset-x-0 top-0 z-50 border-b border-white/[.07] bg-[#080b0a]/82 backdrop-blur-2xl"><div className="mx-auto flex h-[70px] max-w-7xl items-center justify-between px-5 md:px-8"><Brand variant="horizontal" />
