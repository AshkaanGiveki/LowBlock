...
import { z } from "zod";
if (typeof process !== "undefined" && existsSync(".env.local")) {
...
const schema = z.object({
  ...
  GUEST_PREDICTION_SIGNING_SECRET: z.string().min(16).optional(),
  ...
  TELEGRAM_ADMIN_CHAT_ID: z
    ...
    .regex(/^-?\d+$/)
    .optional(),
  TELEGRAM_CHANNEL_ID: z.string().default("@lowblockapp"),
  BALE_API_BASE_URL: z.string().url().default("https://tapi.bale.ai/bot"),
  ...
  FOOTBALL_API_DAILY_LIMIT: z.coerce
    .number()
    .int()
    .positive()
    .max(100)
    .default(100),
  FOOTBALL_API_DETAIL_BATCH_SIZE: z.coerce
    .number()
    .int()
    .positive()
    .max(20)
    .default(20),
  FOOTBALL_API_MODE: z.enum(["current", "season"]).default("current"),
  FOOTBALL_API_SEASON: z.preprocess(
    (value) => (value === "" ? undefined : value),
    z.coerce.number().int().optional(),
  ),
  TRANSFERMARKT_BASE_URL: z
    .string()
    .url()
...
export const env = schema.parse({
  ...
  MONGODB_DIRECT_HOSTS: process.env.MONGODB_DIRECT_HOSTS,
  MONGODB_REPLICA_SET: process.env.MONGODB_REPLICA_SET,
  SESSION_SECRET: safeSecret(
    process.env.SESSION_SECRET,
    "local-session-secret-for-tests",
  ),
  GUEST_PREDICTION_SIGNING_SECRET: safeSecret(
    process.env.GUEST_PREDICTION_SIGNING_SECRET,
    "local-guest-signing-secret",
  ),
  ...
  BALE_API_BASE_URL: process.env.BALE_API_BASE_URL,
  BALE_BOT_TOKEN: process.env.BALE_BOT_TOKEN,
  BALE_BOT_USERNAME: process.env.BALE_BOT_USERNAME,
  ...
  FOOTBALL_API_DETAIL_BATCH_SIZE: process.env.FOOTBALL_API_DETAIL_BATCH_SIZE,
...
export function validateProductionEnv() {
  ...
  const missing = [
    "MONGODB_URI",
    "SESSION_SECRET",
    "GUEST_PREDICTION_SIGNING_SECRET",
