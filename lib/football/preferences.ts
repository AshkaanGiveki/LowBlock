...
export async function getTournamentPreferences(
  userId: string,
): Promise<TournamentPreferences> {
  if (!ObjectId.isValid(userId))
    return {
      favoriteTournamentIds: [...GLOBAL_LEAGUE_CODES],
  ...
  const user = await (
    ...
    .findOne(
      ...
      {
        projection: {
          favoriteTournamentIds: 1,
          favoriteTournamentExclusions: 1,
          showAllMatches: 1,
        },
      },
    );
  ...
  const favorites = [
    ...new Set([
      ...GLOBAL_LEAGUE_CODES.filter((code) => !exclusions.has(code)),
      ...additions,
    ]),
  ];
  return {
    favoriteTournamentIds: favorites,
    showAllMatches: user?.showAllMatches === true,
  };
}

export async function updateTournamentPreference(
  userId: string,
  code: string,
  favorite: boolean,
) {
  if (!ObjectId.isValid(userId) || !knownCodes.has(code))
  ...
  if (favorite) {
    await db
      ...
      .updateOne({ _id: new ObjectId(userId) }, {
        $addToSet: { favoriteTournamentIds: code },
        $pull: { favoriteTournamentExclusions: code },
        $set: { updatedAt: now },
      } as any);
    ...
    await db
      .collection<any>("users")
      .updateOne({ _id: new ObjectId(userId) }, {
        $pull: { favoriteTournamentIds: code },
        $addToSet: { favoriteTournamentExclusions: code },
        $set: { updatedAt: now },
      } as any);
  } else {
    await db
...
export async function setShowAllMatches(
  ...
  await (
    ...
    .updateOne(
      { _id: new ObjectId(userId) },
      { $set: { showAllMatches, updatedAt: new Date() } },
    );
  return getTournamentPreferences(userId);
}
