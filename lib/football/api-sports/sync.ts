...
export async function syncFootballApi() {
  ...
  if (env.FOOTBALL_API_MODE === "season") { for (const league of LEAGUES) { const result = await apiRequest<Fixture>({ league: String(league.apiLeagueId), season: String(env.FOOTBALL_API_SEASON ?? new Date().getUTCFullYear()), timezone: "UTC" }); listRequests++; for (const fixture of result.response) if (league.code !== "FRIENDLY" || isQualityInternationalFriendly(fixture.teams.home.name, fixture.teams.away.name)) byLeague.get(league.code)?.set(fixture.fixture.id, fixture); } }
    ...
    if (currentDate.getUTCHours() === 0) dateKeys.push(new Date(currentDate.getTime() - 86400000).toISOString().slice(0, 10));
  ...
  const shouldSyncInsights = env.FOOTBALL_API_MODE === "season" || new Date().getUTCHours() === 0;
  const insights = shouldSyncInsights ? await syncMatchInsights(db) : { providerRequests: 0, h2hRequests: 0, standingsRequests: 0, snapshots: 0, availableAtStart: 0, matches: 0, providerErrors: [] };
  const rounds = await rebuildRoundRecords(db); await db.collection("syncRuns").insertOne({ provider: "football-api", mode: env.FOOTBALL_API_MODE, startedAt, finishedAt: new Date(), status: "OK", listRequests, detailRequests, fixturesUpdated: total, roundsUpdated: rounds, insights }); const scoreEngine = await runScoreEngine(); invalidateCompetitionCaches(); console.log(`Football API sync complete: ${total} fixtures, ${listRequests + detailRequests + insights.h2hRequests} provider requests; ${rounds} rounds; insight snapshots: ${insights.snapshots}; score engine: ${scoreEngine.scores}`); return { total, listRequests, detailRequests, rounds, insights, scoreEngine };
}
