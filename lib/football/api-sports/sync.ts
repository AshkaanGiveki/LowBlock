...
function normalized(f: Fixture, leagueCode: string) {
  ...
  const matchday = numericRound
    ? Number(numericRound)
    : /round of 16/i.test(round)
      ? 100
  ...
  const status = /FT|AET|PEN/.test(f.fixture.status.short)
    ...
    : /^PST$/.test(f.fixture.status.short)
      ? "POSTPONED"
      : /^SUSP$/.test(f.fixture.status.short)
        ? "SUSPENDED"
        : /CANC|ABD/.test(f.fixture.status.short)
          ? "VOID"
          : /LIVE|1H|2H|HT|ET|BT|P/.test(f.fixture.status.short)
            ? "LIVE"
            : "SCHEDULED";
  return {
    ...
    matchday,
    providerRound: round,
    roundId: `${leagueCode}:${year}:${matchday}`,
    homeTeamProviderId: String(f.teams.home.id),
    awayTeamProviderId: String(f.teams.away.id),
    homeTeam: {
      id: f.teams.home.id,
...
async function saveFixtures(
  ...
  const validFixtures = fixtures.filter((f) => {
    const valid = hasValidFixtureDate(f);
    if (!valid)
      console.error("football_fixture_invalid_date", {
        leagueCode,
  ...
  const teams = fixtures
    ...
    .map((team) => ({
      updateOne: {
        filter: { provider: "football-api", providerTeamId: String(team.id) },
        update: {
          $set: {
            provider: "football-api",
  ...
  if (teams.length)
    await db.collection("teams").bulkWrite(teams, { ordered: false });
  return ops.length;
}

async function saveDailyFixtures(
  dateKey: string,
  db: Awaited<ReturnType<typeof getDb>>,
) {
  ...
  for (const fixture of result.response) {
    const league = LEAGUES.find(
      (item) => item.apiLeagueId === fixture.league.id,
    );
    if (
      !league ||
  ...
  for (const league of LEAGUES) {
    const fixtures = [...(byLeague.get(league.code)?.values() ?? [])];
    total += await saveFixtures(db, league.code, fixtures);
    const season = fixtures.find((fixture) => fixture.league.season)?.league
      .season;
    await db
      .collection("leagues")
      .updateOne(
        { code: league.code },
        {
          $set: {
            ...league,
            seasonStartYear: season ?? new Date().getUTCFullYear(),
            provider: "football-api",
            health: "OK",
            updatedAt: new Date(),
          },
...
export async function syncFootballApiDate(dateKey: string) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(dateKey)) throw new Error("INVALID_UTC_DATE");
...
export async function syncFootballApi() {
  ...
  for (const league of LEAGUES) {
    const fixtures = [...(byLeague.get(league.code)?.values() ?? [])];
    allCurrent.push(...fixtures.filter(hasValidFixtureDate));
    total += await saveFixtures(db, league.code, fixtures);
    const season = fixtures.find((fixture) => fixture.league.season)?.league
      .season;
    await db
  ...
  const now = Date.now();
  const candidates = allCurrent
    .filter((fixture) => {
      const time = new Date(fixture.fixture.date).getTime();
      return time >= now - 864e5 && time <= now + 864e5;
    })
    .map((fixture) => String(fixture.fixture.id));
  const maxDetails = Math.min(await remainingApiRequests(), 95);
  const size = env.FOOTBALL_API_DETAIL_BATCH_SIZE;
  if (env.FOOTBALL_API_MODE === "season")
    for (
      let i = 0;
      i < Math.min(candidates.length, maxDetails * size);
      i += size
    ) {
