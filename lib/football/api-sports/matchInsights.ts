...
function normalizeStanding(row: ProviderStanding) {
  return {
    ...
    goalDifference:
      Number(row.goals?.for ?? 0) - Number(row.goals?.against ?? 0),
  };
}
...
export async function syncMatchInsights(db: Db, options: InsightOptions = {}) {
  ...
  const upcoming = upcomingRows.filter((match) => {
    const kickoff = new Date(match.kickoffAt);
    return Number.isFinite(kickoff.getTime()) && dayKey(kickoff) === today;
  });
  ...
  const pairs = [
    ...new Map(
      upcoming.map((match) => {
        const ids = [
          Number(match.homeTeamProviderId),
          Number(match.awayTeamProviderId),
        ].sort((a, b) => a - b);
  ...
  for (const pairKey of pairs) {
    const match = upcoming.find((candidate) => {
      const ids = [
        Number(candidate.homeTeamProviderId),
        Number(candidate.awayTeamProviderId),
      ].sort((a, b) => a - b);
    ...
    const fresh =
      cached?.updatedAt &&
      now.getTime() - new Date(cached.updatedAt).getTime() <
        24 * 60 * 60 * 1000;
    if (!force && fresh) continue;
    if (providerRequests >= available) break;
    try {
      await throttleProvider();
      const response = await apiRequest<ProviderFixture>(
        { h2h: pairKey },
        "fixtures/headtohead",
      );
      const meetings = response.response
        .filter(
          (item) =>
            finished(item.fixture.status.short) &&
            item.goals.home != null &&
            item.goals.away != null &&
            Number.isFinite(new Date(item.fixture.date).getTime()),
        )
        .sort(
          (a, b) =>
      ...
      providerRequests++;
      providerErrors.push(
        `h2h:${pairKey}:${error instanceof Error ? error.message : "request failed"}`,
      );
  ...
  for (const match of upcoming) {
    ...
    const h2h = isH2HCompetition(match.leagueCode)
      ? await db
          .collection<any>("h2hInsights")
          .findOne({ pairKey: `${ids[0]}-${ids[1]}` })
      : null;
