...
export async function createLockSnapshot(
  db: Db,
  prediction: {
    _id?: unknown;
    userId: string;
  ...
  const existing = await db
    .collection<PredictionLockSnapshot>("predictionLockSnapshots")
  ...
  try {
    await db
      ...
      .insertOne(snapshot as Omit<PredictionLockSnapshot, "_id">);
  } catch (error) {
    if (!(
      error &&
      typeof error === "object" &&
