...
export type LeaderboardRow = Rankable & {
...
function materializedScope(scope: LeaderboardScope) {
  ...
  if (scope.matchday != null)
    return `ROUND:${scope.leagueCode ?? ""}:${scope.seasonStartYear}:${scope.matchday}`;
  if (scope.seasonStartYear != null) return `SEASON:${scope.seasonStartYear}`;
  return "GLOBAL";
}

async function getMaterializedLeaderboard(
  db: Db,
  ...
  if (cursor)
    filter.$or = [
      ...
      { points: cursor.points, exact: { $lt: cursor.exact } },
      {
        ...
        exact: cursor.exact,
        correctOutcome: { $lt: cursor.correctOutcome ?? 0 },
      },
      {
        points: cursor.points,
        exact: cursor.exact,
        predictions: { $lt: cursor.predictions },
      },
      {
        points: cursor.points,
        exact: cursor.exact,
        predictions: cursor.predictions,
        userId: { $gt: cursor.userId },
      },
    ];
  const rows = await db
    .collection<MaterializedRow>("leaderboardStats")
    .find(filter, {
      projection: {
        _id: 0,
        userId: 1,
  ...
  return rows.map((row) => ({
    ...
    correctOutcome: Number(row.correctOutcome ?? 0),
    globalPoints: Number(row.globalPoints ?? 0),
    earliestPredictionAt: row.earliestPredictionAt
      ? new Date(row.earliestPredictionAt).getTime()
      : undefined,
    predictions: Number(row.predictions ?? 0),
    username: byId.get(String(row.userId))?.username ?? "LowBlock Player",
    avatarUrl: byId.get(String(row.userId))?.avatarUrl ?? null,
    isDefendingChampion: String(row.userId) === championId,
  }));
}
...
export async function getCanonicalLeaderboard(
  db: Db,
  scope: LeaderboardScope,
  limit = 100,
  cursor?: {
  ...
  const materialized = await getMaterializedLeaderboard(
    db,
    scope,
    limit,
    cursor,
  ...
  if (materialized)
    return rankRows(materialized).map((row) => ({
      ...row,
      cursor: {
        points: row.points,
        exact: row.exact,
        correctOutcome: row.correctOutcome,
        globalPoints: row.globalPoints,
        earliestPredictionAt: row.earliestPredictionAt,
  ...
  if (scope.clubId) {
    ...
    allowedClubCodes = Array.isArray(club?.leaderboardCompetitionCodes)
  ...
  pipeline.push(
    ...(allowedClubCodes
      ...
      : !scope.clubId && !scope.leagueCode && !scope.matchday
        ? [{ $match: { leagueCode: { $in: GLOBAL_LEAGUE_CODES } } }]
        : []),
    {
      $lookup: {
        from: "predictions",
    ...
    { $match: scoreMatch(scope) },
    {
      $group: {
        ...
        correctOutcome: { $sum: { $cond: ["$correctOutcome", 1, 0] } },
        predictions: { $sum: 1 },
        earliestPredictionAt: { $min: "$prediction.createdAt" },
      },
    },
  );
  if (cursor)
  ...
  const globalRows = globalIds.length
    ? await db
        .collection<any>("leaderboardStats")
        .find(
          { userId: { $in: globalIds }, scope: "GLOBAL" },
          { projection: { userId: 1, points: 1 } },
        )
        .toArray()
    : [];
  const globalPoints = new Map(
    globalRows.map((row) => [String(row.userId), Number(row.points ?? 0)]),
  );
  const ids = rows
  ...
  const mapped = rows.map((row) => ({
    ...
    exact: Number(row.exact ?? 0),
    correctOutcome: Number(row.correctOutcome ?? 0),
    globalPoints: globalPoints.get(String(row._id)) ?? 0,
    earliestPredictionAt: row.earliestPredictionAt
      ? new Date(row.earliestPredictionAt).getTime()
      : undefined,
    predictions: Number(row.predictions ?? 0),
    username: byId.get(String(row._id))?.username ?? "LowBlock Player",
    avatarUrl: byId.get(String(row._id))?.avatarUrl ?? null,
    isDefendingChampion: String(row._id) === championId,
  }));
  return rankRows(mapped).map((row) => ({
    ...row,
    cursor: {
      points: row.points,
      exact: row.exact,
...
export async function getLeaderboardSummary(
  ...
  if (!scopeKey) {
    ...
    return row
      ? {
          ...
          points: row.points,
          exact: row.exact,
          correctOutcome: row.correctOutcome ?? 0,
          predictions: row.predictions,
          scope: "FILTERED",
        }
      : null;
    ...
    .countDocuments({
      ...
      $or: [
        ...
        { points: stats.points ?? 0, exact: { $gt: stats.exact ?? 0 } },
        {
          ...
          exact: stats.exact ?? 0,
          correctOutcome: { $gt: stats.correctOutcome ?? 0 },
        },
        {
          points: stats.points ?? 0,
          exact: stats.exact ?? 0,
          correctOutcome: stats.correctOutcome ?? 0,
          globalPoints: { $gt: stats.globalPoints ?? 0 },
        },
        {
          points: stats.points ?? 0,
          exact: stats.exact ?? 0,
          correctOutcome: stats.correctOutcome ?? 0,
          globalPoints: stats.globalPoints ?? 0,
          earliestPredictionAt: {
            $lt: stats.earliestPredictionAt ?? new Date(8640000000000000),
          },
        },
        {
          points: stats.points ?? 0,
          ...
          globalPoints: stats.globalPoints ?? 0,
          earliestPredictionAt:
            stats.earliestPredictionAt ?? new Date(8640000000000000),
          predictions: { $gt: stats.predictions ?? 0 },
        },
        {
          points: stats.points ?? 0,
          exact: stats.exact ?? 0,
  ...
  return {
    ...
    correctOutcome: Number(stats.correctOutcome ?? 0),
