...
const CHANNEL_PUBLICATION_TYPES = [
  "LEADERBOARD",
  "MATCHES",
  "FIRST_MATCH",
] as const;
...
function utcDayBounds(dateKey: string) {
...
function botLink(path: string) {
  return env.TELEGRAM_BOT_USERNAME
    ? `https://t.me/${env.TELEGRAM_BOT_USERNAME}?startapp=${encodeURIComponent(path)}`
...
async function sendChannelMessage(
  text: string,
  url = `${env.NEXT_PUBLIC_APP_URL}/matches`,
) {
  ...
  const response = await fetch(
    `https://api.telegram.org/bot${env.TELEGRAM_BOT_TOKEN}/sendMessage`,
    {
      ...
      body: JSON.stringify({
        ...
        reply_markup: {
          inline_keyboard: [
            [{ text: "⚽ Predict now", style: "success", url: botLink(url) }],
          ],
        },
      }),
    },
  ...
  if (!response.ok || !body?.ok)
    throw new Error(body?.description || `TELEGRAM_${response.status}`);
  return body.result;
}
function splitTelegramText(text: string) {
  ...
  for (const line of text.split("\n")) {
    ...
    for (let offset = 0; offset < line.length; offset += TELEGRAM_MESSAGE_LIMIT)
      chunks.push(line.slice(offset, offset + TELEGRAM_MESSAGE_LIMIT));
    chunk = "";
  }
  if (chunk) chunks.push(chunk);
  return chunks.length ? chunks : [""];
}
async function sendChannelMessages(
  text: string,
  url = `${env.NEXT_PUBLIC_APP_URL}/matches`,
) {
  const results = [];
  for (const chunk of splitTelegramText(text))
    results.push(await sendChannelMessage(chunk, url));
  return results;
}
async function claimPublication(
  db: Db,
  dateKey: string,
  type: ChannelPublicationType,
) {
  const now = new Date();
  const existing = await db
    .collection<any>("channelPublications")
    .findOne({ dateKey, type }, { projection: { status: 1, updatedAt: 1 } });
  if (!existing) {
    try {
      await db
  ...
  const stale =
    ...
    new Date(existing.updatedAt ?? 0).getTime() >=
      now.getTime() - STALE_PROCESSING_AFTER_MS;
...
export async function publishChannelLeaderboard(db: Db, date = new Date()) {
  ...
  if (!(await claimPublication(db, dateKey, "LEADERBOARD")))
    return { sent: false, reason: "already-published" };
  ...
  const lines = rows.length
    ? rows
        .map(
          (row, index) =>
            `${index + 1}. <b>${escapeHtml(row.username)}</b> — ${row.points} pts`,
        )
        .join("\n")
    : "No leaderboard points yet.";
  try {
    const results = await sendChannelMessages(
      `🏆 <b>LowBlock Global Leaderboard</b>\n📅 ${dateKey}\n\n${lines}`,
      "/leaderboard",
    );
    const messageIds = results.map((result) => result.message_id);
    await markSent(db, dateKey, "LEADERBOARD", messageIds);
    return {
      sent: true,
      messageId: messageIds[0],
      messageIds,
    ...
    await markFailed(db, dateKey, "LEADERBOARD", error);
    throw error;
  }
}
...
export async function publishChannelMatches(db: Db, date = new Date()) {
  const dateKey = utcDateKey(date);
  if (!(await claimPublication(db, dateKey, "MATCHES")))
    return { sent: false, reason: "already-published" };
  ...
  const rows = matches.length
    ? matches
        .map((match) => {
          ...
          return `⚽ <b>${escapeHtml(names.home)} vs ${escapeHtml(names.away)}</b> — ${time} UTC`;
        })
        .join("\n\n")
    : "No matches today.";
  try {
    ...
    const messageIds = results.map((result) => result.message_id);
    await markSent(db, dateKey, "MATCHES", messageIds);
    return {
      sent: true,
      messageId: messageIds[0],
      messageIds,
    ...
    await markFailed(db, dateKey, "MATCHES", error);
    throw error;
  }
}

export async function scheduleFirstMatchChannelReminder(
  db: Db,
  date = new Date(),
) {
  ...
  const match = await db
    ...
    .findOne(
      {
        kickoffAt: {
          $gte: new Date(Math.max(Date.now(), bounds.start.getTime())),
          $lt: bounds.end,
        },
        status: "SCHEDULED",
      },
      {
        sort: { kickoffAt: 1 },
        projection: { providerMatchId: 1, kickoffAt: 1 },
      },
    );
  ...
  await new Client({ token: env.QSTASH_TOKEN }).publishJSON({
    ...
    deduplicationId: `lowblock-channel-first-match-${dateKey}`,
  });
  return true;
}
export async function scheduleChannelDailyPosts(date = new Date()) {
  ...
  const jobs = [
    {
      type: "leaderboard",
      path: "/api/notifications/channel-leaderboard",
      runAt: new Date(bounds.start.getTime() + 8.5 * 3600000),
    },
    {
      type: "matches",
      path: "/api/notifications/channel-matches",
      runAt: new Date(bounds.start.getTime() + 9 * 3600000),
    },
  ];
  let scheduled = 0;
  for (const job of jobs) {
    ...
    await new Client({ token: env.QSTASH_TOKEN }).publishJSON({
      ...
      deduplicationId: `lowblock-channel-${job.type}-${dateKey}`,
    });
    scheduled++;
  }
  return scheduled;
}
export async function publishFirstMatchChannelReminder(
  db: Db,
  dateKey: string,
  matchId: string,
) {
  ...
  if (!match)
    return {
      ...
      polling: await scheduleMatchdayPolling(db, dateKey),
    };
  if (!(await claimPublication(db, dateKey, "FIRST_MATCH")))
    return {
      sent: false,
  ...
  try {
    const result = await sendChannelMessage(
      text,
      `/matches?match=${encodeURIComponent(matchId)}`,
    );
    ...
    await markFailed(db, dateKey, "FIRST_MATCH", error);
    throw error;
  }
}
