...
export async function scheduleMatchdayPolling(db: Db, dateKey: string) {
  if (!LIVE_SCORE_POLLING_ENABLED) return { scheduled: 0, reason: "temporarily-disabled" };
...
export async function pollMatchday(dateKey: string) {
  if (!LIVE_SCORE_POLLING_ENABLED) return { synced: false, reason: "temporarily-disabled" };
  const startedAt = new Date();
  const remaining = await remainingApiRequests();
  if (remaining <= RESERVED_REQUESTS) {
    const now = Date.now(); const { start, end } = utcBounds(dateKey); const matches = await (await getDb()).collection<any>("matches").find({ provider: "football-api", kickoffAt: { $gte: start, $lt: end }, status: { $nin: ["VOID", "CANCELLED", "POSTPONED", "FINISHED"] } }, { projection: { kickoffAt: 1 } }).sort({ kickoffAt: -1 }).limit(1).toArray(); const lastKickoff = matches[0] ? new Date(matches[0].kickoffAt).getTime() : 0; const retryAt = Math.min(now + POLL_RETRY_MS, lastKickoff + POLL_END_AFTER_LAST_KICKOFF_MS); if (retryAt > now && env.QSTASH_TOKEN && env.NEXT_PUBLIC_APP_URL) await new Client({ token: env.QSTASH_TOKEN }).publishJSON({ url: `${env.NEXT_PUBLIC_APP_URL}/api/notifications/matchday-poll`, body: { dateKey }, notBefore: Math.floor(retryAt / 1000), deduplicationId: `lowblock-matchday-retry-${dateKey}-${retryAt}` }); const result = { synced: false, reason: "reserved-quota-only", remaining, retryAt: retryAt > now ? new Date(retryAt).toISOString() : null }; await (await getDb()).collection("footballPollRuns").insertOne({ dateKey, startedAt, finishedAt: new Date(), status: "DEFERRED", ...result }); return result;
  }
  try { const result = { synced: true, remainingBefore: remaining, ...(await syncFootballApiDate(dateKey)) }; await (await getDb()).collection("footballPollRuns").insertOne({ dateKey, startedAt, finishedAt: new Date(), status: "OK", ...result }); return result; } catch (error) { await (await getDb()).collection("footballPollRuns").insertOne({ dateKey, startedAt, finishedAt: new Date(), status: "FAILED", remaining, error: error instanceof Error ? error.message : String(error) }).catch(() => undefined); throw error; }
}
