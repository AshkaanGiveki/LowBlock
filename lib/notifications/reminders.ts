...
const preferenceField = (provider: Provider) =>
  provider === "TELEGRAM"
...
export async function scheduleUpcomingReminders() {
  ...
  const matches = await db
    ...
    .find({ kickoffAt: { $gt: new Date(now) }, status: "SCHEDULED" })
    .toArray();
  let scheduled = 0;
  for (const match of matches) {
    const kickoff = new Date(match.kickoffAt);
    if (!Number.isFinite(kickoff.getTime())) {
      console.error("match_reminder_invalid_kickoff", {
    ...
    try {
      if (await scheduleMatchReminder(match.providerMatchId, runAt))
        scheduled++;
    } catch (error) {
      console.error("match_reminder_scheduling_failed", {
        matchId: match.providerMatchId,
...
export async function processMatchReminder(matchId: string) {
  ...
  for (const identity of identities) {
    const provider = String(identity.provider) as Provider;
    const preferences = identity.preferences?.[0];
    if (preferences?.[preferenceField(provider)] === false) continue;
    if (
      provider === "TELEGRAM" &&
      preferences?.telegramReminderScope !== "ALL_MATCHES" &&
      !isGlobalCompetition(String(match.leagueCode ?? ""))
    )
      continue;
    ...
    const deliveryFilter = {
      ...
      matchId,
      channel,
      type: "MISSING_PREDICTION_30_MIN",
    };
    ...
    if (!reservation.upsertedCount) continue;
    try {
      ...
      await db
        .collection("notificationDeliveries")
        .updateOne(deliveryFilter, {
          $set: {
            status: "SENT",
            sentAt: new Date(),
            externalMessageId: String(
              result?.message_id ?? result?.id ?? "sent",
            ),
            updatedAt: new Date(),
          },
        });
      ...
      await db
        ...
        .updateOne(deliveryFilter, {
          $set: {
            status: "FAILED",
            errorCode:
              error instanceof Error ? error.message : `${provider}_ERROR`,
            updatedAt: new Date(),
          },
        });
    }
...
async function sendPlatform(
  ...
  if (!token) throw Error(`${provider}_NOT_CONFIGURED`);
  const home =
    language === "fa"
      ? readableFa(
          teamName(
  ...
  const text =
    language === "fa"
      ? `⚽ یادآوری پیش‌بینی\n\n${home} مقابل ${away}، ۳۰ دقیقه دیگر شروع می‌شود.\n\nهنوز پیش‌بینی خود را ثبت نکرده‌اید.`
      : `⚽ Prediction reminder\n\n${home} vs ${away} starts in 30 minutes.\n\nYou haven't submitted your prediction yet.`;
  const endpoint =
    provider === "TELEGRAM"
      ? `https://api.telegram.org/bot${token}/sendMessage`
      : `${env.BALE_API_BASE_URL.replace(/\/$/, "")}/${token}/sendMessage`;
  const button = language === "fa" ? "ثبت پیش‌بینی" : "Make prediction";
  const markup =
    provider === "TELEGRAM"
      ? {
          inline_keyboard: [
            [
              {
                ...
                style: "success",
                web_app: {
                  url: `${env.NEXT_PUBLIC_APP_URL}/matches?match=${encodeURIComponent(match.providerMatchId)}`,
                },
              },
            ],
          ],
  ...
  if (!response.ok || !data?.ok)
    throw Error(data?.description || `${provider}_${response.status}`);
  return data.result;
}
