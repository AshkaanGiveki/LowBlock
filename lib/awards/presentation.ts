...
export function getAwardLabel(type: string | undefined, language: "fa" | "en") {
  const labels: Record<string, [string, string]> = {
    ...
    [CLUB_MONTHLY_EXACT_WINNER_TYPE]: [
      "پیش‌بینی‌گر دقیق ماه باشگاه",
      "CLUB EXACT PICKER OF THE MONTH",
    ],
    [MONTHLY_EXACT_WINNER_TYPE]: [
      "پیش‌بینی‌گر دقیق ماه جهانی",
      "GLOBAL EXACT PICKER OF THE MONTH",
    ],
...
export function getAwardPeriod(
  award: { type?: string; roundNumber?: number; periodId?: string },
  language: "fa" | "en",
) {
  if (
    award.type === WEEKLY_WINNER_TYPE ||
    award.type === CLUB_WEEKLY_WINNER_TYPE
  )
    return language === "fa" ? "هفته جاری" : "WEEKLY";
  if (
    award.type === MONTHLY_EXACT_WINNER_TYPE ||
    award.type === CLUB_MONTHLY_EXACT_WINNER_TYPE
  )
    return award.periodId ?? (language === "fa" ? "ماهانه" : "MONTHLY");
  if (award.roundNumber)
    return language === "fa"
      ? `راند ${award.roundNumber}`
      : `MD${String(award.roundNumber).padStart(2, "0")}`;
