...
export type AwardRecord = { _id?: unknown; userId: string; type: typeof ROUND_WINNER_TYPE; scope: typeof LOWBLOCK_SCOPE; competitionId: string; competitionName: string; seasonId: string; seasonStartYear: number; roundId: string; roundNumber: number; awardedAt: Date; revealedAt: Date | null };
export async function grantFinalRoundWinnerAwards(db: Db) {
  ...
  for (const round of rounds) {
    ...
    const result = await db.collection<AwardRecord>("awards").updateOne({ userId: winner.userId, type: ROUND_WINNER_TYPE, scope: LOWBLOCK_SCOPE, competitionId: round.leagueCode, seasonId: String(round.seasonId), roundId: String(round.id) }, { $setOnInsert: { userId: winner.userId, type: ROUND_WINNER_TYPE, scope: LOWBLOCK_SCOPE, competitionId: round.leagueCode, competitionName: asset.leagueName, seasonId: String(round.seasonId), seasonStartYear, roundId: String(round.id), roundNumber: Number(round.number), awardedAt: new Date(), revealedAt: null } }, { upsert: true });
