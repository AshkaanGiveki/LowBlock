...
async function grantMonthlyExactAward(db: Db) {
  ...
  const rank = async (groupId: string | Record<string, string>) => {
    ...
    return rows.map(row => ({ row, userId: String(typeof row._id === "object" ? row._id.userId : row._id), globalPoints: global.get(String(typeof row._id === "object" ? row._id.userId : row._id)) ?? 0 })).sort((a, b) => Number(b.row.exact ?? 0) - Number(a.row.exact ?? 0) || Number(b.row.correct ?? 0) - Number(a.row.correct ?? 0) || b.globalPoints - a.globalPoints || (a.row.earliest ? new Date(a.row.earliest).getTime() : Number.MAX_SAFE_INTEGER) - (b.row.earliest ? new Date(b.row.earliest).getTime() : Number.MAX_SAFE_INTEGER) || a.userId.localeCompare(b.userId));
  };
  const rows = await rank("$userId"); const winner = rows[0]; let granted = winner ? await issue(db, { awardKey: `MONTHLY_EXACT_WINNER:${period.id}`, userId: winner.userId, type: MONTHLY_EXACT_WINNER_TYPE, scope: LOWBLOCK_SCOPE, competitionName: "Monthly Exact", periodId: period.id }) : 0;
  const activeClubs = await eligibleClubIds(db);
  const clubRows = await rank({ clubId: "$clubIdAtLock", userId: "$userId" }); const awardedClubs = new Set<string>(); for (const entry of clubRows) { const clubId = String(entry.row._id.clubId); if (awardedClubs.has(clubId) || !activeClubs.has(clubId)) continue; awardedClubs.add(clubId); granted += await issue(db, { awardKey: `CLUB_MONTHLY_EXACT_WINNER:${clubId}:${period.id}`, userId: entry.userId, type: CLUB_MONTHLY_EXACT_WINNER_TYPE, scope: "CLUB", competitionName: "Club Monthly Exact", periodId: period.id, clubId, exact: entry.row.exact, points: entry.row.points, predictions: entry.row.predictions }); }
  return granted;
}
