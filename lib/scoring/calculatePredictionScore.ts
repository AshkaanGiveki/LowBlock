...
): { points: number; category: ScoreCategory } {
  if (
    ...
    return { points: 0, category: "NO_PREDICTION" };
  if (predictedHomeGoals === homeGoals && predictedAwayGoals === awayGoals)
    return { points: 10, category: "EXACT_SCORE" };
  const outcome = (h: number, a: number) => (h === a ? "D" : h > a ? "H" : "A");
  const po = outcome(predictedHomeGoals, predictedAwayGoals),
    ro = outcome(homeGoals, awayGoals);
  if (
    po === ro &&
    ro !== "D" &&
