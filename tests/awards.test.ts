...
import {
  CLUB_LEAGUE_WINNER_ASSET,
  CLUB_EXACT_PICKER_ASSET,
  CLUB_ROUND_ASSETS,
  CLUB_WEEKLY_ASSET,
  CLUB_MONTHLY_EXACT_WINNER_TYPE,
  GLOBAL_LEAGUE_WINNER_ASSET,
  GLOBAL_EXACT_PICKER_ASSET,
  getAwardAsset,
  getAwardAssetContentType,
  getRoundWinnerAsset,
...
describe("round winner awards", () => {
  it("maps each supplied first-release asset to the intended competition and round", () => {
    expect(getRoundWinnerAsset("GB1", 2026, 1)?.leagueName).toBe(
      "Premier League",
    );
    expect(getRoundWinnerAsset("ES1", 2026, 2)?.leagueName).toBe("LaLiga");
    expect(
      getRoundWinnerAsset("L1", 2026, 1)
        ?.trophy.replaceAll("\\", "/")
        .endsWith("round/global/L1/Trophy.png"),
    ).toBe(true);
    ...
    expect(getRoundWinnerAsset("GB1", 2026, 2)?.trophy).toBe(
      getRoundWinnerAsset("GB1", 2026, 1)?.trophy,
    );
    expect(
      getRoundWinnerAsset("ES1", 2026, 1)
        ?.trophy.replaceAll("\\", "/")
        .endsWith("round/global/ES1/Trophy.webp"),
    ).toBe(true);
  ...
  it("maps every award type to one canonical design family", () => {
    ...
    expect(getAwardAsset({ type: "CLUB_WEEKLY_WINNER" })?.trophy).toBe(
      CLUB_WEEKLY_ASSET.trophy,
    );
    expect(
      getAwardAsset({
        type: "ROUND_WINNER",
    ...
    ).toBe(getRoundWinnerAsset("ES1", 2026, 1)?.trophy);
    expect(
      getAwardAsset({
        type: "CLUB_ROUND_WINNER",
        competitionId: "L1",
        seasonStartYear: 2026,
        roundNumber: 99,
      })?.trophy,
    ).toBe(CLUB_ROUND_ASSETS.L1.trophy);
    expect(getAwardAsset({ type: MONTHLY_EXACT_WINNER_TYPE })?.trophy).toBe(
      GLOBAL_EXACT_PICKER_ASSET.trophy,
    );
    expect(
      getAwardAsset({ type: CLUB_MONTHLY_EXACT_WINNER_TYPE })?.trophy,
    ).toBe(CLUB_EXACT_PICKER_ASSET.trophy);
    expect(getAwardAsset({ type: GLOBAL_WINNER_TYPE })?.trophy).toBe(
      GLOBAL_LEAGUE_WINNER_ASSET.trophy,
    );
    expect(getAwardAsset({ type: "CLUB_LEAGUE_WINNER" })?.trophy).toBe(
      CLUB_LEAGUE_WINNER_ASSET.trophy,
    );
  });
  ...
  it("uses the application's canonical points/exact/prediction/user-id ordering", () => {
    const [winner] = rankRows([
      { userId: "b", points: 10, exact: 1, predictions: 3 },
      { userId: "a", points: 10, exact: 1, predictions: 3 },
    ]);
