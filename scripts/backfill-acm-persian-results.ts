  ...
  try {
    ...
    for (const membership of activeMemberships)
      await db
        ...
        .updateMany(
          { userId: membership.userId, clubIdAtLock: clubId },
          { $set: { membershipIdAtLock: String(membership._id) } },
        );
    const scores = await db
      .collection("predictionScores")
      .countDocuments({ clubIdAtLock: clubId });
    console.log(
      JSON.stringify(
        {
          club: club.name,
          activeMembers: activeMemberships.length,
          scoresUpdated: scoreResult.modifiedCount,
          snapshotsUpdated: snapshotResult.modifiedCount,
          clubScores: scores,
          scoreEngine,
        },
        null,
