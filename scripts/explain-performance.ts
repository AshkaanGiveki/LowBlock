...
void (async () => {
  ...
  try {
    ...
    const reports: Record<string, unknown> = {
      ...
      leaderboardStats: await db
        .collection("leaderboardStats")
        .find({ scope: "GLOBAL" })
        .sort(leaderboardSort)
        .limit(20)
        .explain("executionStats"),
      predictionScores: await db
        .collection("predictionScores")
        .find({ seasonStartYear: new Date().getUTCFullYear() })
        .sort({ kickoffAt: 1 })
        .limit(20)
        .explain("executionStats"),
      detailedLeaderboard: await db
        .collection("predictionScores")
        .aggregate([
          {
            $match: {
              seasonStartYear: new Date().getUTCFullYear(),
              leagueCode: { $in: ["39", "2", "3", "848", "135", "78"] },
            },
          },
          {
            $group: {
    ...
    if (club)
      reports.clubLeaderboard = await db
        .collection("leaderboardStats")
        .find({ scope: { $regex: `^CLUB:${String(club._id)}:` } })
        .sort(leaderboardSort)
        .limit(20)
        .explain("executionStats");
    ...
    if (viewer)
      reports.currentUserRank = await db
        .collection("leaderboardStats")
        .find({
          scope: "GLOBAL",
          $or: [
            { points: { $gt: viewer.points ?? 0 } },
            { points: viewer.points ?? 0, exact: { $gt: viewer.exact ?? 0 } },
            {
              points: viewer.points ?? 0,
              exact: viewer.exact ?? 0,
              correctOutcome: { $gt: viewer.correctOutcome ?? 0 },
            },
            {
              points: viewer.points ?? 0,
              exact: viewer.exact ?? 0,
              correctOutcome: viewer.correctOutcome ?? 0,
              globalPoints: { $gt: viewer.globalPoints ?? 0 },
            },
          ],
        })
        .explain("executionStats");
